    ...
    <div className="flex min-h-dvh min-h-screen flex-col overflow-x-hidden bg-white">
      <main className="flex min-h-0 flex-1 flex-col items-center justify-center gap-4 px-4 py-6 sm:gap-6 sm:p-8 md:gap-8">
        <Image
          ...
          className="h-auto w-full max-w-[160px] sm:max-w-[200px] md:max-w-[240px] lg:max-w-[280px]"
        />

        {entityName && (
          <a
            href={entityBackHref}
            className="flex items-center gap-3 rounded-full border border-gray-100 bg-white px-4 py-2 shadow-sm transition-colors hover:bg-gray-50"
          >
            <span
        ...
        <div className="module-buttons-group mx-auto flex w-full max-w-[260px] flex-col items-center justify-center gap-4 sm:max-w-[400px] sm:flex-row sm:flex-nowrap sm:gap-6 md:max-w-[820px]">
          <ModuleButton
            iconSrc="/pettycash-icon.webp"
            iconAlt="Petty cash"
            imageScale={0.8}
            hoverBackImage="/minty-l.webp"
            onClick={handleClick}
            onTouchEnd={handleTouchEnd}
          />
          <ModuleButton
            iconSrc="/payment-icon.webp"
            iconAlt="Payment request"
            imageScale={1.0}
            hoverBackImage="/minty-r.webp"
            hoverBackImagePosition="top-right"
            onClick={handlePaymentClick}
            onTouchEnd={handlePaymentTouchEnd}
          />
        </div>
      </main>

      <div
        className={`pointer-events-none fixed bottom-0 left-1/2 z-[100] block h-[260px] w-[280px] max-w-[85vw] -translate-x-1/2 transition-transform duration-300 ease-out md:hidden ${showMinty ? "translate-y-[25%]" : "translate-y-full"}`}
        aria-hidden
      >
        <Image
          src="/minty.webp"
