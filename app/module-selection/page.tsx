...
const MODULE1_URL = process.env.NEXT_PUBLIC_MODULE1_URL ?? "http://192.168.1.60:5001";
const MODULE2_BACKEND_URL = process.env.NEXT_PUBLIC_MODULE2_BACKEND_URL ?? "http://127.0.0.1:8000";
...
function ModuleSelectionContent() {
  ...
  const module1Href = entityId && token
      ...
      <main className="flex min-h-0 flex-1 flex-col items-center justify-center gap-4 px-4 py-6 sm:gap-6 sm:p-8 md:gap-8">
        ...
        {entityName && (
          <a href={entityBackHref} className="flex items-center gap-3 rounded-full border border-gray-100 bg-white px-4 py-2 shadow-sm transition-colors hover:bg-gray-50">
            <span className="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-[11px] font-bold" style={{ backgroundColor: "#E0F7FA", color: "#00838F" }}>{acronym}</span>
            <span className="text-sm font-semibold text-[#474747]">{entityName}</span>
          </a>
        )}
